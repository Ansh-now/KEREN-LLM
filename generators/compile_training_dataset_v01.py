#!/usr/bin/env python3
"""Compile validated KEREN Gold V0.1 traces into QLoRA-ready JSONL.

The Gold files remain the source of truth. This compiler creates a smaller
runtime-learning representation without training on internal labels such as
should_confirm/should_recover.

Output rows match training/train_qlora.py:
    {"id": "...", "input": "...", "target": {...}}
"""
...
DEFAULT_OUTPUT = ROOT / "datasets" / "compiled" / "keren_train_v0.1.jsonl"
...
def _clean(value: Any) -> Any:
    """Recursively remove null/empty values while preserving booleans/numbers."""
...
def build_target(record: dict) -> dict:
    ...
    mode = final_output.get("mode") or (record.get("task") or {}).get("output_mode") or "answer"
    ...
    # For user-facing/gated flows, the Gold response text is the primary target.
    ...
    # Action mode teaches compact execution routing while keeping the final
    # user-facing text when Gold explicitly contains one.
    target.update(
        {
...
def main() -> int:
    ...
    mode_summary = ", ".join(f"{k}={v}" for k, v in sorted(modes.items()))
